...
const targetTypeConfig: Record<NoticeTargetType, { label: string; icon: React.ReactNode; color: string }> = {
  ...
  DEPARTMENT: { label: "Department", icon: <Building2 className="w-3.5 h-3.5" />, color: "bg-secondary text-foreground border-border" },
  SUB_DEPARTMENT: { label: "Sub-Department", icon: <Network className="w-3.5 h-3.5" />, color: "bg-secondary text-foreground border-border" },
  INDIVIDUAL: { label: "Individual", icon: <User className="w-3.5 h-3.5" />, color: "bg-secondary text-foreground border-border" },
}
...
export default function AdminNotificationsPage() {
  ...
  const [targetType, setTargetType] = React.useState<NoticeTargetType>("ALL")
  ...
  const handlePublish = async () => {
    ...
    if (!editingNoticeId) {
      if (targetType === "DEPARTMENT" && !targetId) return toast.error("Select a department")
      if (targetType === "SUB_DEPARTMENT" && !targetId) return toast.error("Select a sub-department")
      if (targetType === "INDIVIDUAL" && selectedUserIds.length === 0) return toast.error("Select at least one staff member")
    }
    ...
    try {
      ...
      if (editingNoticeId) {
        ...
        const dto: CreateBroadcastNoticeDto = {
          ...
          targetType,
          isPinned,
          ...(targetType === "DEPARTMENT" || targetType === "SUB_DEPARTMENT" ? { targetId } : {}),
        ...
        <Dialog open={createOpen} onOpenChange={(o) => { setCreateOpen(o); if (!o) resetForm() }}>
          ...
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingNoticeId ? "Update existing notice details" : "Create and publish notices to staff members"}
              </DialogDescription>
            </DialogHeader>
                    ...
                    <Select value={targetType} onValueChange={(v) => { setTargetType(v as NoticeTargetType); setTargetId(undefined); setSelectedUserIds([]) }}>
                      ...
                      <SelectContent>
                        <SelectItem value="ALL">
                          <span className="flex items-center gap-2"><Users className="w-4 h-4" /> All Staff</span>
                        </SelectItem>
                        <SelectItem value="DEPARTMENT">
                          <span className="flex items-center gap-2"><Building2 className="w-4 h-4" /> Department</span>
                        </SelectItem>
                        <SelectItem value="SUB_DEPARTMENT">
                          <span className="flex items-center gap-2"><Network className="w-4 h-4" /> Sub-Department</span>
                        </SelectItem>
                ...
                {!editingNoticeId && targetType === "SUB_DEPARTMENT" && (
                  <div className="space-y-2">
                    ...
                    <Select value={targetId?.toString() || ""} onValueChange={(v) => setTargetId(Number(v))}>
                      ...
                      <SelectContent>
                        {targets.subDepartments.map(sd => (
                          <SelectItem key={sd.id} value={sd.id.toString()}>
                            {sd.name} — {sd.department?.name}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
